<template>
  <div class="member-home">
    ...
    <HomePanel title="我的收藏">
      <GoodsItem v-for="item in collectGoods" :key="item" :goods="item" />
    </HomePanel>
    <!-- 足迹 -->
    <HomePanel title="我的足迹">
      <GoodsItem v-for="item in likeGoods" :key="item" :goods="item" />
    </HomePanel>
    <!-- 猜你喜欢 -->
    <GoodsRelevant />
  </div>
...
import GoodsItem from '@/views/category/components/good-item.vue'
import { findCollect, findLike } from '@/api/member'
import { ref } from 'vue'
...
export default {
  ...
  setup () {
    ...
    const likeGoods = ref([])
    findLike({ page: 1, pageSize: 4 }).then((data) => {
      likeGoods.value = data.result.items
    })
    return { collectGoods, likeGoods }
  }
}
</script>
